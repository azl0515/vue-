...
router.post('/', (req, res, next) => {
  ...
  req.files && req.files.forEach((file, index) => {
    ...
    if (file.fieldname === 'follow') {
      follow = '/upload/follow/' + file.filename + pathLib.parse(file.originalname).ext;
    }
  ...
  if (!follow) follow = '/upload/default.jpg'
  ...
  mgdb.open({
    ...
    ({ collection, client }) => {
      ...
      let data = { title, sub_title, follow, time, detail: { icon, auth, content } }
    ...
    ({ collection, client, ObjectId }) => {
      collection.find({
        ...
        collection.updateMany({
          ...
          $set:{
            title,sub_title,follow,time,detail:{icon,auth,content}
          }
        },{
          upsert:false,
          projection:false

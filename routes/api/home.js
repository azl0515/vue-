...
router.post('/', (req, res, next) => {
  ...
  req.files && req.files.forEach((file, index) => {
    ...
    if (file.fieldname === 'home') {
      home = '/upload/home/' + file.filename + pathLib.parse(file.originalname).ext;
    }
  ...
  if (!home) home = '/upload/default.jpg'
  ...
  mgdb.open({
    ...
    ({ collection, client }) => {
      ...
      let data = { title, sub_title, home, time, detail: { icon, auth, content } }
    ...
    ({ collection, client, ObjectId }) => {
      collection.find({
        ...
        collection.updateMany({
          ...
          $set:{
            title,sub_title,home,time,detail:{icon,auth,content}
          }
        },{
          upsert:false,
          projection:false

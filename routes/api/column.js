...
router.post('/', (req, res, next) => {
  ...
  req.files && req.files.forEach((file, index) => {
    ...
    if (file.fieldname === 'column') {
      column = '/upload/column/' + file.filename + pathLib.parse(file.originalname).ext;
    }
  ...
  if (!column) column = '/upload/default.jpg'
  ...
  mgdb.open({
    ...
    ({ collection, client }) => {
      ...
      let data = { title, sub_title, column, time, detail: { icon, auth, content } }
    ...
    ({ collection, client, ObjectId }) => {
      collection.find({
        ...
        collection.updateMany({
          ...
          $set:{
            title,sub_title,column,time,detail:{icon,auth,content}
          }
        },{
          upsert:false,
          projection:false
